...
const extractor = () => {
  ...
  return {
    extract: function extract(knex: IKnex) {
      return co(function*() {
        ...
        const db = {
          ...
          dataTypes: _.keyBy(
            getRows(yield cmds.getDataTypes(knex)),
            "typeName"
          ),
          schemas: {},
        };

        const versionInfo = getRows(
          yield cmds.getVersionInfo(knex)
        )[0].version.split(" ");
        ...
        if (db.variant.productVersion && db.variant.productVersion.length > 0) {
          db.variant.majorVersion = `PostgreSQL ${
            db.variant.productVersion.split(".")[0]
          }`;
        }
        ...
        for (const schemaName of schemaNames) {
          ...
          const schemaTableColumns = _.map(
            getRows(yield cmds.getTableColumns(knex, schema.name)),
            (row) => {
              const col = {
                table: row.table,
                name: row.name,
          ...
          _.forEach(schema.tables, (table) => {
            table.columns = _.keyBy(
              _.filter(schemaTableColumns, { table: table.name }),
              "name"
            );
            // remove the table property
            _.forEach(table.columns, (col) => {
              _.unset(col, "schema");
              _.unset(col, "table");
            });
          });
          ...
          const schemaViewColumns = _.map(
            getRows(yield cmds.getViewColumns(knex, schema.name)),
            (row) => {
              return {
                view: row.view,
                name: row.name,
          ...
          _.forEach(schema.views, (view) => {
            view.columns = _.keyBy(
              _.filter(schemaViewColumns, { view: view.name }),
              "name"
            );
            // remove the view property
            _.forEach(view.columns, (col) => {
              _.unset(col, "schema");
              _.unset(col, "view");
            });
          });
        ...
        for (const schemaName of schemaNames) {
          ...
          const schemaColumnDescriptions = getRows(
            yield cmds.getColumnDescriptions(knex, schema.name)
          );

          _.forEach(schemaColumnDescriptions, (scd) => {
            if (scd.description && scd.description.length > 0) {
              schema.tables[scd.table].columns[scd.name].description =
                scd.description;
            }
          });

          // get check constraints
          const checkConstraints = getRows(
            yield cmds.getCheckConstraints(knex, schema.name)
          );
          if (checkConstraints.length > 0) {
            _.forEach(schema.tables, (table) => {
              const tableCheckConstraints = _.filter(checkConstraints, {
                table: table.name,
              });
              if (tableCheckConstraints.length > 0) {
                table.checkConstraints = _.keyBy(tableCheckConstraints, "name");
                _.forEach(table.checkConstraints, (ck) => {
                  _.unset(ck, "schema");
                  _.unset(ck, "table");
                });
              }
          ...
          const primaryKeys = {};
          if (indexes.length > 0) {
            _.forEach(_.groupBy(indexes, "name"), (idxArr, idxName) => {
              const idx = idxArr[0];
          ...
          const primaryKeyConstraints = getRows(
            yield cmds.getPrimaryKeyConstraints(knex, schema.name)
          );
          if (primaryKeyConstraints.length > 0) {
            _.forEach(_.groupBy(primaryKeyConstraints, "table"), (
              pk,
              pkName
            ) => {
              const table = schema.tables[pkName];
              table.primaryKey = {
                name: primaryKeys[pkName],
                columnCount: pk.length,
                columns: _.map(pk, "column"),
              };
              _.forEach(pk, (pkc) => {
          ...
          const uniqueKeyConstraints = getRows(
            yield cmds.getUniqueKeyConstraints(knex, schema.name)
          );
          if (uniqueKeyConstraints.length > 0) {
            _.forEach(_.groupBy(uniqueKeyConstraints, "name"), (
              uk,
              ukName
            ) => {
              ...
              const ukk = {
                name: ukName,
                columnCount: uk.length,
                columns: _.map(uk, "column"),
              };
              table.uniqueKeys = table.uniqueKeys || {};
              table.uniqueKeys[ukName] = ukk;
              _.forEach(ukk.columns, (ukc) => {
          ...
          const foreignKeyConstraints = getRows(
            yield cmds.getForeignKeyConstraints(knex, schema.name)
          );
          if (foreignKeyConstraints.length > 0) {
            schema.foreignKeys = _.keyBy(
              _.map(foreignKeyConstraints, (fkc) => {
                return {
                  name: fkc.name,
            ...
            _.forEach(schema.foreignKeys, (fk) => {
              const fkc =
                schema.tables[fk.fkTableName].columns[fk.fkColumnName];
          ...
          const functions = _.map(
            getRows(yield cmds.getFunctions(knex, schemaName)),
            (row) => {
              return {
                name: row.name,
          ...
          const functionsWithArgumentInfo = getRows(
            yield cmds.getFunctionArguments(knex, schemaName)
          );
          if (
            functionsWithArgumentInfo &&
            functionsWithArgumentInfo.length > 0
          ) {
            _.forEach(functionsWithArgumentInfo, (
              functionWithArgumentInfo
            ) => {
              ...
              const parameterDefinitions = (
                functionWithArgumentInfo.parameterDefinitions || ""
              ).split(", ");
              const pcount = parameterDefinitions.length;
              const parameterNames =
                functionWithArgumentInfo.parameterNames || [];
              while (parameterNames.length < pcount) {
                // create a fake name for the argument
                parameterNames.push(`$${parameterNames.length}1`);
              }
              const parameterModesStr =
                functionWithArgumentInfo.parameterModes || "";
              const parameterModes =
                parameterModesStr.length > 2
                  ? parameterModesStr
                      .substring(1, parameterModesStr.length - 2)
                      .split(",")
                  : [pcount];
              ...
              const inDataTypes = (
                functionWithArgumentInfo.inDataTypes || ""
              ).split(" ");
              while (inDataTypes.length < pcount) {
                inDataTypes.push(null);
              }
              const allDataTypes = functionWithArgumentInfo.allDataTypes || [
                pcount,
              ];
              const pargs: any[] = [];
              for (let p = 0; p < pcount; p++) {
                ...
                const pIsInParam =
                  pMode == null || pMode === "i" || pMode !== "b";
                const pIsOutParam =
                  (pMode != null && pMode === "o") ||
                  (pDef || "").split(" ")[0] === "OUT";
                const pDataTypeOid =
                  (pIsInParam && inDataTypes[p]) || allDataTypes[p] || pDef;
